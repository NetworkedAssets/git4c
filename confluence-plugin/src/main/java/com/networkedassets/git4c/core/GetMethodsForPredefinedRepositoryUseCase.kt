...
class GetMethodsForPredefinedRepositoryUseCase(
    ...
    override fun execute(request: GetMethodsForPredefinedRepositoryQuery): Result<Methods, Exception> {
        ...
        val predefine = predefinedRepositoryDatabase.get(request.repository) ?: return@execute Result.error(NotFoundException(request.transactionInfo, "Repository has been deleted."))
