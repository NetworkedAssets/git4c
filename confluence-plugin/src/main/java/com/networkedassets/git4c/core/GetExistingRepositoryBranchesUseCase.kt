...
class GetExistingRepositoryBranchesUseCase(
    ...
    override fun execute(request: GetExistingRepositoryBranchesQuery): Result<Branches, Exception> {
        val repository = repositoryDatabase.get(request.repositoryUuid) ?: return@execute Result.error(NotFoundException(request.transactionInfo, "Repository has been deleted."))
