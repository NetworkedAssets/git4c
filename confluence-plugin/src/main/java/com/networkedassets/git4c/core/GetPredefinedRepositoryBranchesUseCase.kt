...
class GetPredefinedRepositoryBranchesUseCase(
    ...
    override fun execute(request: GetPredefinedRepositoryBranchesQuery): Result<Branches, Exception> {
        val predefine = predefinedRepositoryDatabase.get(request.predefinedRepository) ?: return@execute Result.error(NotFoundException(request.transactionInfo, VerificationStatus.REMOVED))
