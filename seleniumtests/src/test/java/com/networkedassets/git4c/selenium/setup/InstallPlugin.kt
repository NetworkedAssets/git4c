...
class InstallPlugin {
    ...
    fun installPlugin() {
        ...
        val (request, response, result) = Fuel.get("""http://localconfluence:8090/rest/plugins/1.0/?os_authType=basic""").authentication().basic("admin", "admin").responseString()

        val token = response.headers["upm-token"]!!.first()
        println(request)
        println(result)

        val res2 = Fuel.delete("""http://localconfluence:8090/rest/plugins/1.0/$pluginId-key""").authentication().basic("admin", "admin").responseString().third
